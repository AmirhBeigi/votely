...
export const Option: React.FC<OptionProps> = props => {
  ...
  useEffect(() => {
    setTimeout(() => setWidthBar(percentage ?? 0), 0);
  }, [percentage]);
  ...
  return (
    ...
    >
      <Box
        className="bg-black h-20 absolute left-0 top-0 rounded-lg flex items-center justify-end transition-all w-0 duration-1000 shadow-[0_15px_48px_0_rgba(53,62,69,0.3)] z-10"
      ...
      <Text
        dir="auto"
        fontSize="xs"
        className="z-10 mr-2 mix-blend-difference text-white text-opacity-60 transition-all duration-300"
        style={
          percentage === null
            ? {
                marginLeft: '-3.5rem',
      ...
      <Text
        dir="auto"
        fontWeight="normal"
        className="w-full z-10 mix-blend-difference text-white line-clamp-1"
      >
        {title}
      </Text>
    </Box>
